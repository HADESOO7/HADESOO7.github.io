...
const ContactForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Prevent bot submissions completely without network request
    if (honey) {
      toast({
        title: "Message Sent!",
    ...
    // Client-side rate limiting (1 minute)
  ...
  return (
    <form className="min-w-7xl mx-auto sm:mt-4" onSubmit={handleSubmit}>
      <input type="text" name="_honey" className="absolute opacity-0 -left-[9999px]" aria-hidden="true" value={honey} onChange={(e) => setHoney(e.target.value)} tabIndex={-1} autoComplete="off" />
