...
const AnimatedBackground = () => {
  ...
  const handleMouseHover = (e: SplineEvent) => {
    ...
    if (e.target.name === "body" || e.target.name === "platform") {
      ...
      if (!selectedSkill || selectedSkill.name.toLowerCase() !== e.target.name.toLowerCase()) {
        const matchingKey = Object.keys(SKILLS).find(key => key.toLowerCase() === e.target.name.toLowerCase());
        const skill = matchingKey ? SKILLS[matchingKey as SkillNames] : undefined;
  ...
  const revealKeyCaps = async () => {
    ...
    // Debug step to see what names the keycap objects actually have
  ...
  const getKeycapsAnimation = () => {
    ...
    const start = () => {
      ...
      Object.values(SKILLS)
        ...
        .forEach((skill, idx) => {
          const allNamed = splineApp.getAllObjects().filter(o => o.name === skill.name);
          const keycap = allNamed.find(o => (o as any).parent?.name?.startsWith("row")) || allNamed[0];
          if (!keycap) return;
